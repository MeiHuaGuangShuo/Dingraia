此文档最后更新于 `v2.1.0-Pre`。发行版和当前仓库版本可能存在明显差别，本文档所展示内容均基于最新仓库文件
...
[AI生成用文档](./dingraia_ai_guide.md)

`Stream连接快速部署`用于部署机器人，`AI生成用文档`用于给AI实现快速制作可用的模块

有问题可以优先将这两个文档丢给AI，然后附上问题/报错文本，大部分情况下AI能为你解决

注：由于钉钉策略调整，**免费版组织**API和Webhook/Stream用量调整至 **5000** 和 **3000** 。
为了机器人能够持久运行，请尽量**不要开启不相关/不需要的Webhook并调整调用措施**
